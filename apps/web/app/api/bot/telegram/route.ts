...
/**
 * PATCH /api/bot/telegram/bots/:id/status
 * 更新机器人状态
 */
export async function PATCH(req: NextRequest) {
  try {
    ...
    const isStatusUpdate = pathParts.includes('status');

    if (!botId || !isStatusUpdate) {
      return errorResponse('无效的请求路径', 400);
    }
    ...
    try {
      ...
      }, `机器人状态已更新为 ${body.status}`);
    } catch (error) {
      await session.endSession();
      if (error instanceof Error && error.message === 'BOT_NOT_FOUND') {
        return errorResponse('未找到指定的Bot', 404);
      }
      throw error;
    }
  } catch (error) {
    console.error('更新机器人状态失败:', error);
    return errorResponse('更新机器人状态失败');
  }
} 